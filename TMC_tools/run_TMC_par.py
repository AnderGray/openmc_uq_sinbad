...
def prepSim(mats, sets, geo, index):
    ...
    command = "openmc -s 1"
    os.system(command)
    shutil.move(f"statepoint.{Ninner}.h5", f"{statepoint_dir}/statepoint.{index}.h5")
    os.chdir(script_dir)
    shutil.rmtree(ThisSim)
...
with Pool() as pool:
    ...
    for i in range(1,Nouter+1):
        func_args = (copy.deepcopy(materials), copy.deepcopy(settings), copy.deepcopy(geometry), i)
        r = pool.apply_async(prepSim,func_args)
        results.append(r)
